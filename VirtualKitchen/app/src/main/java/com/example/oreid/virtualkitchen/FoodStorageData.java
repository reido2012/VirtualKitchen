...
public class FoodStorageData implements ChildEventListener {
    ...
    /**
     * Adds all food item to fridge from shoppinglist
     * @param index where in the database the item to add is
     */
    public void shoppingListItemToStorage(int index) {
        FoodItem f = get(SHOPPINGLIST).get(index);
        add(new FoodItem(f.getName(), f.getQty(), FRIDGE,f.getShelfLife()));
        remove(f);
        updateLists(SHOPPINGLIST);
    }

    /**
     * Adds single food item to shoppinglist from a storage location
     * @param index where in the database the item to add is
     * @param StorageArea storage location to take item from
     */
    public void storageItemToShoppingList(int index, StorageArea s) {
        FoodItem f = get(s).get(index);
        add(new FoodItem(f.getName(), 1, SHOPPINGLIST, f.getShelfLife()));
        updateLists(s);
    }
